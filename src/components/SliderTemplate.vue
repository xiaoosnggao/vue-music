<template>
  <div class='slider-container' :class='basicdata.containerClass'>
    <div class="slider-wrapper" :style="styleobj" :class="basicdata.animation" @touchmove="swipeMove" @touchstart="swipeStart" @touchend="swipeEnd" @mousedown="swipeStart" @mouseup="swipeEnd"
         @mousemove="swipeMove">
      <!-- 正常滚动 -->
      ...
      <template v-if="sliderinit.loop">
        ...
        <template v-for="item in pages">
          <div class="slider-item"><img v-bind:src="item.src" alt=""></div>
        </template>
        <div class="slider-item"></div>
      </template>
    </div>
